...
多轮能力当前由平台的 Conversation Service 以简化规则提供，不要求 Provider 原生支持会话。

> 实现状态（2026-07-23）：当前版本不引入 LLM 规划器或 Agent 模式。每轮任务固定查询同一会话最近一次成功任务的首张结果，有则作为唯一历史图片上下文提交；该规则不再依赖“上一张、保持、继续”等关键词。完整边界见[《多轮会话历史图片上下文说明》](./多轮会话历史图片上下文问题说明.md)。
...
本轮用户消息
    ...
    + 会话级默认参数
        ↓ Context Builder
文本 Prompt 与图片输入分离的单次生成或编辑请求
```

- 每轮均为独立 Provider 请求，不维护上游会话标识，也不宣称为 Provider 原生多轮。
- Context Builder 将历史文本、本轮上传图和固定选择的一张历史 Asset 转换为一次独立请求。
- Web 端不提供“引用历史图片”按钮；同一会话存在成功图片时自动续接，完全不参考旧图时当前通过新建会话实现。
- 图片不会作为文字拼接进 Prompt，而是通过对应 Provider Adapter 转换为独立的多模态图片输入。只有风格模板和 Prompt 自动增强文本进入最终 Prompt。
- 历史图片固定选择同一会话最近创建的 `succeeded` 任务中 `result_index` 最小的首张结果；不解析“第一张、第二张”等自然语言指代。
- 本轮上传图照常保留，历史上下文额外最多一张；同一 Asset 已被显式选择时不重复追加，合计最多 10 张。
...
- 上下文不使用固定“20 条”限制。当前简化版沿当前分支保留最近 8 条非空用户/助手文本；原始消息和图片仍完整保存在本地历史中。
...
```text
ghcr.io/kodechicken/ai-image-studio@sha256:...
```
...
| 修改 | 简化版多轮上下文固定携带同一会话最近成功任务的首张结果，不再依赖关键词；图片作为独立输入 | Context Builder、任务请求快照、`task_input_images` |
